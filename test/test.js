...
describe('Integration Tests', () => {
  ...
  it('should create an account and validate account creation, then fail on duplicate account creation', async function() {
    ...
    let createResponse = await request.post('/v2/user').send(userData);
    ...
    const getResponse = await request.get('/v2/user/self').auth(userData.username, userData.password);
    ...
    createResponse = await request.post('/v2/user').send(userData);
  ...
  it('should update the account and validate the updates', async function() {
    ...
    if (updateResponse.status === 200) {
      ...
      const getResponse = await request.get('/v2/user/self')
        .auth(userData.username, userData.password);
